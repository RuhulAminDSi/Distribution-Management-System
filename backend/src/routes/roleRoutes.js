...
import { validateCreateRole, validateUpdateRolePermissions } from '../utils/validation.js';
...
router.post('/', authenticate, permit('roles_manage'), validateCreateRole, async (req, res, next) => {
  try {
    await roleController.createRole(req, res, next);
    clearPermissionCache();
  } catch (err) { next(err); }
});
router.put('/:id', authenticate, permit('roles_manage'), validateUpdateRolePermissions, async (req, res, next) => {
  try {
